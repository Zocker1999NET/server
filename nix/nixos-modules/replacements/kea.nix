...
let
  ...
  buildConfig =
    servName: confName: servCfg:
    chooseNotNull servCfg.configFile (
      format.generate "kea-${servName}.conf" { "${confName}" = servCfg.settings; }
    );

  ctrlAgentConfig = buildConfig "ctrl-agent" "Control-agent" cfg.ctrl-agent;

  dhcp4Config = buildConfig "dhcp4" "Dhcp4" cfg.dhcp4;

  dhcp6Config = buildConfig "dhcp6" "Dhcp6" cfg.dhcp6;

  dhcpDdnsConfig = buildConfig "dhcp-ddns" "DhcpDdns" cfg.dhcp-ddns;
